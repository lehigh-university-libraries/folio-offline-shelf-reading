...
function scanNextBarcode() {
  ...
  const scannedRow = itemBarcodes.indexOf(barcode) + 1;
  if (scannedRow < 1) {
    alert("row not found");
  }
  else if (scannedRow == expectedRow) {
    ...
    const shelfStatusInput = getInputAtRow(scannedRow);
    shelfStatusInput.value = SHELF_STATUS_PRESENT;
    for (row = expectedRow; row < scannedRow; row++) {
      const shelfStatusInput = getInputAtRow(row);
      shelfStatusInput.value = SHELF_STATUS_MISSING;
    }
    setExpectedRow(scannedRow + 1);
  }
  else {
    beep("The scanned item was misplaced; it should be shelved earlier in this range.\n\nPlease re-shelve it now and then click OK.")
    const shelfStatusInput = getInputAtRow(scannedRow);
    shelfStatusInput.value = SHELF_STATUS_PRESENT;
  }
